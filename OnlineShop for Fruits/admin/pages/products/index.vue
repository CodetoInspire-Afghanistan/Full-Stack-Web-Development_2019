          ...
          <tbody>
            <tr v-for="(product, index) in products" :key="product._id">
              ...
              <td>
                ...
                <a @click="onviewProduct(product._id)" class="custom-btn openDeleteModal" data-toggle="modal" data-target="#deleteModal"><i class="fa fa-trash" style="color: #dc3545"></i></a>
              </td>
            </tr>
          </tbody>
        </table>
              ...
              <div class="row">
                ...
                <div class="col-9">
                  ...
                  <div class="form-group row pt-3">
                    <label for="description" class="col-sm-4"><b>Description</b></label>
                    <div class="col-sm-8">
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content"  v-if="productDetails">
          ...
          <div class="modal-body">
            Are you sure
              <br>
            Do you want to <span style="color:red">delete</span> this?
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">No</button>
            <button @click="onDeleteProduct(productDetails._id)" type="button" class="btn btn-danger">Yes</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  async asyncData({$axios}) { //asyncData -->this method related to NuxtJs & Before loading page asyncData feach data and use for calling API
    try {
      const categoryResponse = await $axios.$get('http://localhost:3000/api/categories') //We want to fetch data from category so at the first call (await)
      ...
      console.log("owner-----",ownerResponse.owners)
      return {
        categories: categoryResponse.categories, // the secound categories is same with category rout in server
  ...
  methods: {
    ...
    async onDeleteProduct(id) {
      try {
        ...
        const updateProduct = await this.$axios.$get('http://localhost:3000/api/products')
        this.products = updateProduct.products // Update product without refreshing
        
        $('#deleteModal').modal('hide') // for closeing Modal
        // if(result.status) {
    ...
    async productUpdate(id) {
      try {
        ...
        $('#editModal').modal('hide') // for closeing Modal
